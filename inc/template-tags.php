...
if ( ! function_exists( 'jazzclub_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function jazzclub_posted_on() {
...
if ( ! function_exists( 'jazzclub_posted_by' ) ) :
	/**
...
if ( ! function_exists( 'jazzclub_entry_footer' ) ) :
	/**
	...
	function jazzclub_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'jazzclub' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'jazzclub' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'jazzclub' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'jazzclub_post_thumbnail' ) ) :
	 ...
	 */
	function jazzclub_post_thumbnail() {
